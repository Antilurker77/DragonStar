...
std::string Ab_WaterBolt::GetDescription() {
	std::string damage = "16";

	if (user != nullptr) {
		damage = std::to_string(Combat::ScalingAttackEstimate(user, 16.0, Attribute::INT, abilityOptions));
	}
...
void Ab_WaterBolt::execute(std::vector<ActorPtr>& targets, sf::Vector3i cursor, std::vector<sf::Vector3i> targetArea) {
	Combat::ScalingAttack(user, targets[0], 16.0, Attribute::INT, abilityOptions);
}