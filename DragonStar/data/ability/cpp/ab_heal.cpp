...
std::string Ab_Heal::GetDescription() {
	std::string healing = "30";

	if (user != nullptr) {
		healing = std::to_string(Combat::ScalingHealEstimate(user, heal, Attribute::HP, abilityOptions));
	}
...
void Ab_Heal::execute(std::vector<ActorPtr>& targets, sf::Vector3i cursor, std::vector<sf::Vector3i> targetArea) {
	Combat::ScalingHeal(user, targets[0], heal, Attribute::HP, abilityOptions);
}